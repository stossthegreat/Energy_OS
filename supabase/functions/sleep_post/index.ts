...
Deno.serve(async req=>{
  try{
    ...
    const { data: ins, error } = await admin.from("sleep_logs").upsert(row, { onConflict: "user_id,date" }).select().single();
    if (error) return json(500,{error:error.message});

    // trigger energy update
    const fu = `${SUPABASE_URL}/functions/v1/energy_update`;
    fetch(fu,{method:"POST",headers:{Authorization:`Bearer ${SERVICE_KEY}`,"Content-Type":"application/json"},body:JSON.stringify({source:"sleep"})}).catch(()=>{});
